...
def test_translate_texts_preserves_acronyms_and_source_language(monkeypatch):
    ...
    mock_client.translate_text.return_value = SimpleNamespace(
        translations=[
            SimpleNamespace(
                translated_text=(
                    "Plan de calor para [[PAC_ACRONYM_0]] con mejoras de [[PAC_ACRONYM_1]]"
                )
            )
        ]
    )
    ...
    result = client.translate_texts(
        ["Plan de calor para CDP con mejoras de HVAC"],
        target_language="en",
        source_language="es",
    )

    assert result == ["Plan de calor para CDP con mejoras de HVAC"]
