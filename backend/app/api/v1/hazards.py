...
@router.get("/layer-config", response_model=HazardLayerConfigResponse)
def get_hazard_layer_config() -> HazardLayerConfigResponse:
    """
    Retrieve the configuration for all hazard layers.

    Returns:
        A HazardLayerConfigResponse containing the configuration for each hazard layer.
    """
    config = get_hazard_config()
    return HazardLayerConfigResponse(config=config)


@router.get("/{hazard_type}", response_model=HazardLayerResponse)
def get_ee_hazard_layer(
    ...
    try:
        ...
        layer = ee_hazard_data_provider.fetch_hazard_layer(
            hazard_type, scenario, year_range
        )
        return HazardLayerResponse(layer=layer)
