...
* ### Cloud SQL
  ...
  If the source data is already staged in BigQuery, export each table to CSV with columns in the same order as `backend/scripts/create_empty_tables.sql`, then load through the proxy with `psql`:
